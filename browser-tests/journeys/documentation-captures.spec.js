...
const SOCCERNET = "SoccerNet Match Intelligence · synthetic docs fixture";
...
test("capture current workbench journeys with redistributable fixtures", async ({
  page,
  workbench,
}) => {
  ...
  await page.goto(await workbench.start("documentation"), {
  ...
  await expect(
    page.getByText(/Deterministic, redistributable BDD100K/),
  ).toBeVisible();
  ...
  await page.getByLabel("Labels").fill("vehicle, pedestrian");
  await page
    .getByLabel("Review note")
    .fill("Reviewed synthetic road-scene fixture");
  await page.getByRole("button", { name: "Add bounding box" }).click();
  await page.getByRole("button", { name: "Save annotation" }).click();
  await expect(page.locator(".annotation-box")).toBeVisible();
  await capture(page, "03-annotation-editor.png");

  await openDestination(page, "Training");
  await page
    .getByRole("button", {
      name: "Train synthetic BDD100K Road Scene Lab adapter",
    })
    .click();
  await expectCompleted(page);
  await openDestination(page, "Training");
  await expect(page.locator(".line-chart > i")).toHaveCount(3);
  await capture(page, "04-training-dashboard.png");

  await openDestination(page, "Inference");
  await page
    .getByRole("button", { name: "Run synthetic vision fixture", exact: true })
    .click();
  await expectCompleted(page);
  await page
    .getByRole("button", { name: /result\.mp4/ })
    .first()
    .click();
  ...
  await chooseProject(page, SOCCERNET);
  await openDestination(page, "Dataset");
  await page.getByRole("option", { name: /Authored match clip/i }).click();
  await openDestination(page, "Inference");
  await page
    .getByRole("button", { name: "Run synthetic SoccerNet inference" })
    .click();
  await expectCompleted(page);
  await openDestination(page, "Jobs / Runs");
  await capture(page, "06-soccernet-jobs.png");

  await chooseProject(page, QWEN);
  await openDestination(page, "Inference");
  const prompt =
    "Explain why keeping experiment results attached to saved runs is useful.";
  await page.getByRole("textbox", { name: "Prompt" }).fill(prompt);
  await capture(page, "04-qwen-prompt.png");

  await page
    .getByRole("button", { name: "Run synthetic prompt fixture", exact: true })
    .click();
  await expectCompleted(page);
  await page
    .getByRole("button", { name: /^assistant\.txt/ })
    .first()
    .click();
  await expect(page.getByLabel("Checked assistant response")).toContainText(
    prompt,
  );
  ...
  await page
    .getByRole("button", {
      name: "Run synthetic base-SigLIP fixture",
      exact: true,
    })
    .click();
  await expectCompleted(page);
