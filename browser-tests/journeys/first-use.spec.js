...
test("restored workbench presents the complete established shell and real project source", async ({page, workbench}) => {
  ...
  await expect(page).toHaveScreenshot("first-use-desktop.png", {
    fullPage: true,
    animations: "disabled",
    maxDiffPixels: 1_000,
    mask: [page.locator(".execution-provider"), page.locator(".execution-health"), page.locator(".bottom-panel pre")],
  });
});
