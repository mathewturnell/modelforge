...
| Local annotation | Planned | Server-owned mutations and held-out protections must pass before controls are enabled. |
...
| Training/evaluation/run comparison | Planned | Must reuse the durable lifecycle and prevent held-out evidence from entering iterative selection. No current training or model-quality claim. |
