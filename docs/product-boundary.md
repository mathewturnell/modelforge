...
| Project/source/Git | Partial | Registration, bounded read-only source/file inspection, and read-only Git status are supported. Editing and confirmed Git mutation remain disabled gaps. |
| Architecture/model inspection | Supported | Safe structural projection of registered actions, dataset inputs, checked-result contracts, and bindings without importing project code or deserializing checkpoints. Arbitrary project-supplied architecture descriptors remain excluded. |
...
| Compiled React loopback workbench | Partial | The reviewed established shell now projects real overview, source, dataset, box and conversation annotation/curation, structural model graph, registered training, inference/prompt, Jobs, logs, failure, cancellation, recovery, checked image/video/table results, settings, and assistant services. Source/Git writes, arbitrary project creation/import, LLM evaluation, release building, and deployment remain disabled. HTTP remains unstable. |
...
| Local ModelForge Coding Assistant/Codex | Supported, read-only | The embedded panel uses the OS user's authenticated Codex account through a clean Codex App Server adapter. ModelForge owns exact-project/session/run identity, normalized streamed events, durable answers, and cancellation while Codex owns credentials. There is no deterministic answer fallback. Attachments, delegation, write authority, Git, paid actions, publication, and deployment remain disabled. |
| Local settings | Partial | Exposes server-owned non-secret workbench/Modal status plus bounded Codex account status and provider-owned ChatGPT login. Broader local and commercial controls remain planned. |
