...
| Local ModelForge Coding Assistant/Codex | Partial | The restored Cliff panel uses a durable, project-scoped, read-only local evidence assistant with cancellation and no credential/held-out access. A model-backed Codex provider and attachment/delegation tools remain disabled pending separate isolation qualification. |
