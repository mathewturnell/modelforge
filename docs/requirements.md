...
| `PUB-ARCH-002` | partial | Every enabled local product feature is exposed through a typed service port and additive `/api/v1` projection before its React control is enabled; source, model inspection, and assistant ports remain unavailable. | API schema/route tests; feature rows below |
