...
| `PUB-PROJ-001` | partial | The local UI supports registered-project switching, bounded source inspection, and read-only Git status through server-owned paths. Browser project creation/import, source editing, and confirmed Git operations remain disabled. | project/source/Git service and browser tests |
