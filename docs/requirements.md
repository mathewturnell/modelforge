...
| `PUB-AGENT-001` | implemented | ModelForge Coding Assistant uses the OS user's authenticated Codex account through a provider-neutral, exact-project-scoped, durable, cancellable Codex App Server adapter. The public slice is read-only, normalizes streamed public events, retains no provider credential or raw provider object, and has no deterministic response fallback. Attachments, delegation, write authority, Git, paid actions, publication, and deployment remain disabled. | Codex protocol fixture; assistant service/security/browser tests; Decision 0003 |
| `PUB-SET-001` | partial | Settings exposes server-owned browser-security and Modal readiness plus bounded Codex App Server account status and provider-owned ChatGPT login without credential values. Broader local and commercial configuration remains planned. | Settings browser tests; provider route tests; Decision 0003 |
