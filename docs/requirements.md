...
| `PUB-DATA-001` | partial | Dataset catalogs and authenticated checked sample access use the shared project service; local annotation mutations are bounded and cannot alter protected held-out material. | existing dataset tests; new annotation service/tests |
| `PUB-MODEL-001` | planned | Architecture/model inspection validates descriptors and safe structural evidence without importing arbitrary project code or deserializing unsafe checkpoints. | descriptor/inspection tests; React graph tests |
| `PUB-TRAIN-001` | planned | Local training, validation, and compatible run comparison reuse durable Run/Executor/Artifact ownership and never expose held-out evaluation for iteration. | training/evaluation service and negative tests |
