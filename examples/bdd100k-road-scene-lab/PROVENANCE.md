...
The six Python files were compared individually with the exact upstream
MeMOTR/MOTR trees. The entry point, package initializer, contract validator, and
mapping module appear independently authored for ModelForge; their Apache-2.0
licensing remains blocked on confirmation of the underlying contributor and
relicensing rights.
...
`MIT AND Apache-2.0 AND LicenseRef-ModelForge-Pending` because the exact
upstream file carries the MeMOTR → MOTR → Deformable-DETR/DETR attribution
chain. `inference_runtime.py` is `MIT AND LicenseRef-ModelForge-Pending`.
Applicable upstream notices are retained in `THIRD_PARTY_NOTICES`; ownership of
the ModelForge additions still requires confirmation. See
